...
local function getClosestPlayer()
    ...
    for _,v in next, Players:GetPlayers() do
        if v ~= Lp and v.Character and v.Character:FindFirstChild("Humanoid") then
            if v.Character.Humanoid.Health ~= 0 and v.Character:FindFirstChild(AimSettings.TargetPart) then
                local Target = v.Character[AimSettings.TargetPart]
                local Pos, OnScreen = Cam:WorldToScreenPoint(Target.Position)
                if OnScreen then
                    if (Vec(Pos.X, Pos.Y) - Vec(Mouse.X, Mouse.Y)).Magnitude < AimSettings.Radius then
                        Close = v.Character[AimSettings.TargetPart]
                    end
                end
            end
        end
...
local function Esp(v)
    ...
    InnerBox.Transparency = 0.4
    InnerBox.Thickness = 1
    ...
    game:GetService'RunService'.RenderStepped:Connect(function()
        if EspSettings.Enabled and v.Character ~= nil and v.Character:FindFirstChild("HumanoidRootPart") ~= nil and v.Character:FindFirstChild("Humanoid") ~= nil and v.Character:FindFirstChild("Head") ~= nil and v.Character.Humanoid.Health ~= 0 then
            ...
            local Size = (Cam:WorldToViewportPoint(RootPart.Position - Vector3.new(0, 3, 0)).Y - Cam:WorldToViewportPoint(RootPart.Position + Vector3.new(0, 2.6, 0)).Y) / 2
            ...
            if onScreen then
                ...
                if EspSettings.Names then
                    ...
                    Names.Text = v.Name
                    Names.Visible = true
                    Names.Position = Vector2.new(BoxSize.X / 2 + BoxPos.X, BoxPos.Y - 16)
                else
                    Names.Visible = false
                end
            else
...
local function esp_dealer(obj, text)
    ...
    game:GetService'RunService'.RenderStepped:Connect(function()
        ...
        if EspSettings.CustomNPCs.Dealer then
            local Vector, onScreen = Cam:WorldToScreenPoint(obj.Position)
...
for i,v in next, NPCFolder:GetChildren() do
    if v:IsA("Model") and v.PrimaryPart ~= nil then
        esp_dealer(v.PrimaryPart, "Dealer")
    end
end
...
local mainTab = {
    mainsec = Tabs.maintab:CreateSector("Client", "left"),
    aimsec = Tabs.maintab:CreateSector("Silent Aim", "right")
}
...
visualTab.espsec:AddToggle("Enabled", false,function(s)
    EspSettings.Enabled = s
end)
visualTab.espsec:AddToggle("Draw Names", false,function(s)
    EspSettings.Names = s
end)
visualTab.espsec:AddToggle("Draw Boxes",false,function(s)
    EspSettings.Boxes = s
end)
visualTab.espsec:AddToggle("Inside Box",false,function(s)
    EspSettings.InnerBox = s
end)

visualTab.espsec:AddSeperator("")
